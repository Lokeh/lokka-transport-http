...
const checkIsNode = new Function('try {return this===global;}catch(e){return false;}');

let fetchUrl;
let isNode = checkIsNode();
...
export class Transport extends LokkaTransport {
  ...
  _buildOptions(payload) {
    ...
    if (this.agent && isNode) {
      // Allow node clients to pass a custom agent to node-fetch
      // to allow e.g. custom proxies, certs, etc.
